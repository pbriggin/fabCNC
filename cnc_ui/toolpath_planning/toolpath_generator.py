...
class ToolpathGenerator:
    ...
    def __init__(self, 
        ...
        Args:
            cutting_height: Z height when cutting in mm (more negative = lower)
            safe_height: Z height when raised in mm (less negative = higher)
        ...
        # Directory for saving gcode output files
    ...
    def generate_toolpath(self, shapes: Dict[str, List[Tuple[float, float]]], source_filename: str = None, notches: dict = None) -> str:
        ...
        # Save gcode to file
        self._save_gcode_file(gcode_str, source_filename)
        
        return gcode_str
    
    def _save_gcode_file(self, gcode: str, source_filename: str = None) -> str:
        """
        Save generated GCODE to a file.
        
        Args:
            gcode: The GCODE string to save
            source_filename: Optional source filename to use for naming
    ...
    def _generate_header(self) -> List[str]:
        """Generate GCODE header for Marlin firmware."""
        return [
            "; Fabric CNC Toolpath",
            "; Generated by ToolpathGenerator",
            "; Firmware: Marlin",
            "",
            "M919 X O4 P2 S1   ; Set X chopper timing to 24V defaults",
            "M919 Y O4 P2 S1   ; Set Y chopper timing to 24V defaults",
            "",
    ...
    def _calculate_z_rotation(self, point1: Tuple[float, float], point2: Tuple[float, float]) -> float:
        ...
        Returns:
            A-axis position in degrees (for Marlin firmware)
        """
        ...
        # Convert to degrees (positive = counterclockwise from X-axis)
        angle_degrees = math.degrees(angle_radians)
        ...
        # Return angle in degrees directly (Marlin uses degrees for rotary axes)
        a_position_degrees = adjusted_angle
    ...
    def _calculate_continuous_a(self, target_a: float) -> float:
        ...
        Args:
            target_a: Target A position in degrees (0.0 to 360.0)
            
        Returns:
            Continuous A position in degrees that takes shortest path from current position
        """
        # Calculate the difference between target and current (normalized to 0-360 range)
        diff = target_a - (self.current_a % 360.0)
        
        # If difference is greater than 180, it's shorter to go the other way
        if diff > 180.0:
            # Go backwards (subtract 360)
            continuous_a = self.current_a + diff - 360.0
        elif diff < -180.0:
            # Go forwards (add 360)
            continuous_a = self.current_a + diff + 360.0
        else:
